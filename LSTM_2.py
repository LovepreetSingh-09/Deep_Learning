# -*- coding: utf-8 -*-
"""
Created on Tue Sep 10 21:21:09 2019
...
class LSTMcell(Layer):
    def __init__(self,n_inputs,n_hidden,n_outputs):
        ...
        self.xu=Linear(n_inputs,n_hidden)
        self.xo=Linear(n_inputs,n_hidden)
        
        self.hf=Linear(n_hidden,n_hidden,bias=False)
        self.hi=Linear(n_hidden,n_hidden,bias=False)
        self.hu=Linear(n_hidden,n_hidden,bias=False)
        self.ho=Linear(n_hidden,n_hidden,bias=False)
        
        self.wo=Linear(n_hidden,n_outputs,bias=False)
        ...
        self.parameters+=self.wo.get_parameters()
    
    def forward(self,input,hidden):
        prev_h=hidden[0]
        prev_cell=hidden[1]
        f=(self.xf.forward(input)+self.hf.forward(prev_h)).sigmoid()
        i=(self.xi.forward(input)+self.hi.forward(prev_h)).sigmoid()
        u=(self.xu.forward(input)+self.hu.forward(prev_h)).tanh()
        o=(self.xo.forward(input)+self.ho.forward(prev_h)).sigmoid()
        cell=(f*prev_cell)+(i*u)
        h=o*cell.tanh()
        output=self.wo.forward(h)
...
input_=indices[:batch_size*n_batches]
input_=input_.reshape(batch_size,n_batches)
input_=input_.transpose()
input_batches=input_[0:-1]
target_batches=input_[1:]
n_bptt=int((n_batches-1)/bptt)
input_batches=input_batches[:n_bptt*bptt]
input_batches=input_batches.reshape(n_bptt,bptt,batch_size)
target_batches=target_batches[:n_bptt*bptt]
...
for i in range(n_iters):
    ...
    for batch_i in range(len(input_batches)):
        ...
        for t in range(bptt):
            ...
            lstm_input=embed.forward(input=input)
            output,hidden=model.forward(input=lstm_input,hidden=hidden)
            ...
            if t==0:
                ...
                loss.append(batch_loss+loss[-1])
        loss_=loss[-1]
        loss_.backward()
        total_loss+=loss_.data/bptt
        ...
        if(batch_i == 0):
            log += " - " + generate_samples(n=70, init_char='T').replace("\n"," ")
        if(batch_i % 1 == 0):
            sys.stdout.write(log)
    optim.alpha *= 0.99
        
        
print(generate_samples(n=500,init_char='\n'))
